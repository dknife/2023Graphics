...
class MyGLWidget(QOpenGLWidget):
    ...
    def paintGL(self):  
        ...
        if self.bDisplayListMode:
            glColor3f(0,1,1)
            glCallList(self.myDisplayList)
        else:
            glColor3f(1,1,0)
            drawPlane()  # immediate mode

    def toggleDisplayMode(self):
        if self.bDisplayListMode == True:
            self.bDisplayListMode = False
        else:
            self.bDisplayListMode = True
...
class MyWindow(QMainWindow):
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key.Key_W:
            ...
            self.glWidget.toggleDisplayMode()
            self.glWidget.update()
