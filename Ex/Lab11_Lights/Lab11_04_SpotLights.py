...
lit_position0 = [-20, 5, 10, 1]
lit_position1 = [ 20, 5, 10, 1]
lit_position2 = [  0, 5, 10, 1]

spot_direction = [0, -1, -0.3]
def LightSet():
...
class MyGLWidget(QOpenGLWidget):
    ...
    def initializeGL(self):
        ...
        self.myLoader.loadMesh("./Lab11_Lights/cow.txt", reverseNormal=True)
        self.angle = 0
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        LightSet()
## >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
